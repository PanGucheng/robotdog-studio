  ...
  async runTurn(context: AdapterTurnContext, emit: (event: AdapterEvent | unknown) => void, signal: AbortSignal): Promise<{ summary: string }> {
    ...
    try {
      ...
      const result = await process.client.request<{ stopReason: string }>('session/prompt', {
        sessionId,
        prompt: [{ type: 'text', text: context.readOnly ? context.message : buildStudentAgentPrompt(context.message, { policyVersion: context.policyVersion, trustedCourseContext: context.courseContext, templateId: context.templateId, templateVersion: context.templateVersion, firmwareBaselineId: context.firmwareBaselineId, workspacePurpose: context.workspacePurpose }) }]
      }, 10 * 60_000)
