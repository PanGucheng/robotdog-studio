...
export class WorkspaceService {
  ...
  private async writeManagedProjectFiles(projectRoot: string): Promise<void> {
    const policy = {
      schemaVersion: 1,
      policyProfile: 'student-v1',
      allowedPaths: ['Core/Src/**/*.c', 'Core/Inc/**/*.h', 'student-config/**/*.yaml'],
      deniedPaths: ['**/.git/**', '**/startup*', '**/*.ld', 'reasonix.toml', 'AGENTS.md'],
      maxChangedFiles: 12,
      maxChangedBytes: 96_000
    }
  ...
  private toSummary(metadata: WorkspaceMetadata): WorkspaceSummary {
    const { id, name, studentDisplayName, templateId, templateVersion, lastCheckpoint: headCommit, state, updatedAt } = metadata
    return { id, name, studentDisplayName, templateId, templateVersion, headCommit, state, updatedAt }
  }
}
