...
import { BookOpen, CheckCircle2, ChevronDown, ChevronRight, CircleAlert, Code2, File, FileCode2, FileJson2, FileSliders, Folder, FolderOpen, FolderTree, Hammer, LockKeyhole, Play, RotateCcw, Save, ShieldCheck, Sparkles, Zap } from 'lucide-react'
...
export function StudentCodeEditor({ workspace, candidate, busy, onCandidateChanged, onReadyForReview, onRejectCandidate, onExplainCode, diagnosticHelp: _diagnosticHelp, onRepairStudentCode: _onRepairStudentCode, explorerMode = false, focusRequest, onActiveFileChange, editorOverlay, overlayVisible = false, bottomPanel, workspaceAction, workspaceNotice, workspaceDiagnostics = [], railAction }: StudentCodeEditorProps): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    if (saveTimer.current) clearTimeout(saveTimer.current)
    saveTimer.current = setTimeout(() => { void saveCurrent() }, MCU_AUTO_SAVE_DELAY_MS)
  ...
  return (
    <div className="student-code-studio">
      ...
      <div className="student-editor-main">
        ...
        {diagnostic ? <div className="editor-feedback has-error"><strong>代码检查发现问题</strong><p>{diagnostic}</p>{manualCandidate && <button type="button" onClick={() => requestDiagnosticHelp(manualCandidate.id, buildDiagnostics, diagnostic)}>让 AI 解释</button>}</div>
          : workspaceNotice ? <div className={`editor-feedback ${workspaceNotice.tone === 'error' ? 'has-error' : ''}`}><strong>{workspaceNotice.title}</strong><p>{workspaceNotice.text}</p></div>
