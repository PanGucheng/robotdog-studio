import { ArrowLeft, ArrowRight, BookOpenCheck, Clock3, Code2, FlaskConical, Plus } from 'lucide-react'
...
export function McuHome({ panel, course, workspaces, learning, recent, busy, onNavigate, onCreateWorkspace }: McuHomeProps): React.JSX.Element {
  ...
  if (panel === 'free-practice') {
    ...
    return <section className="mcu-free-home">
      ...
      <header><span className="eyebrow">FREE WORKSHOP</span><h1>自由练习</h1><p>基于 Pony v2.5 全功能小马固件编写自己的控制程序，尝试传感器、运动和机器人行为。</p></header>
      <button type="button" className="mcu-create-free" onClick={onCreateWorkspace} disabled={busy}><span><Plus size={22} /></span><strong>创建自由练习工程</strong><small>基于 Pony v2.5 全功能小马固件，可编写控制逻辑、读取 CCD、控制运动行为。</small><ArrowRight size={18} /></button>
      <div className="mcu-recent-list"><h2>最近自由练习</h2>{free.length > 0 ? free.map((workspace) => <button type="button" key={workspace.id} onClick={() => onNavigate({ kind: 'workspace', workspaceId: workspace.id })}><Code2 size={17} /><span><strong>{workspace.name}</strong><small>{workspace.templateId === 'ch32v203-pony' ? '完整小马固件 · Pony v2.5' : '自由练习'} · {formatTime(workspace.updatedAt)}</small></span><ArrowRight size={15} /></button>) : <p>还没有自由练习工程。创建一个工程后，可以随时从这里继续。</p>}</div>
    </section>
  }
  ...
  return <section className="mcu-home">
    <header className="mcu-home-hero"><div><BrandLogo /><h1>今天想从哪里开始？</h1><p>先理解原理，再走进实验；也可以直接打开自由工作室。</p></div><img className="brand-motif" src={brandMotif} width="180" height="90" alt="" /></header>
    <div className="mcu-path-choices">
      ...
      <button type="button" className="is-free" onClick={() => onNavigate({ kind: 'home', panel: 'free-practice' })}><span><Code2 size={25} /></span><small>FREE WORKSHOP</small><strong>自由练习</strong><p>基于 Pony v2.5 全功能小马固件自由编写、构建和测试机器人行为。</p><b>开始练习 <ArrowRight size={15} /></b></button>
    </div>
