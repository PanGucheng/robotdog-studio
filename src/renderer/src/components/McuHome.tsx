...
export function McuHome({ panel, course, workspaces, learning, recent, busy, onNavigate, onCreateWorkspace }: McuHomeProps): React.JSX.Element {
  ...
  if (panel === 'free-practice') {
    ...
    return <section className="mcu-free-home">
      ...
      <header><span className="eyebrow">FREE WORKSHOP</span><h1>自由练习</h1><p>基于完整小马固件编写自己的控制程序，尝试传感器、运动和机器人行为。</p></header>
  ...
  return <section className="mcu-home">
    ...
    <div className="mcu-path-choices">
      ...
      <button type="button" className="is-free" onClick={() => onNavigate({ kind: 'home', panel: 'free-practice' })}><span><Code2 size={25} /></span><small>FREE WORKSHOP</small><strong>自由练习</strong><p>基于完整小马固件自由编写、构建和测试机器人行为。</p><b>开始练习 <ArrowRight size={15} /></b></button>
    </div>
