import { BookOpenCheck, CheckCircle2, ChevronLeft, Cpu, FolderTree, Hammer, PanelLeft, X } from 'lucide-react'
import { useEffect, useRef, useState, type CSSProperties, type PointerEvent as ReactPointerEvent } from 'react'
import type { CourseLesson, FirmwareUpdateSnapshot, StudentCodeExplanationRequest, WchLinkFlashSnapshot } from '../../../shared/types'
...
export function McuWorkbench(props: WorkbenchProps): React.JSX.Element {
  ...
  const projectTourCheckedRef = useRef(false)
  ...
  useEffect(() => {
    ...
    if (projectTourCheckedRef.current) return
    projectTourCheckedRef.current = true
    if (localStorage.getItem(PROJECT_TOUR_SEEN_KEY)) { setTourOpen(false); return }
    localStorage.setItem(PROJECT_TOUR_SEEN_KEY, '1')
    setTourIndex(0)
    setTourOpen(true)
  ...
  const diffLayer = showDiff && candidate ? <div className="mcu-diff-surface"><header><span><FolderTree size={15} /> 修改确认</span><div className="mcu-diff-header-actions"><button type="button" onClick={() => props.onRejectCandidate(candidate.id)} disabled={busy}><X size={14} /> 放弃修改</button>{candidate.state === 'build_passed' ? <button type="button" className="button-primary" onClick={() => props.onApplyCandidate(candidate.id)} disabled={busy}><CheckCircle2 size={14} /> {busy ? '正在保存…' : '保存到项目'}</button> : <><button type="button" onClick={() => props.onBuildCandidate(candidate.id)} disabled={busy || candidate.state !== 'review_ready'}><Hammer size={14} /> {busy ? '正在检查…' : '检查代码'}</button><button type="button" className="button-primary" onClick={() => props.onApplyCandidate(candidate.id)} disabled={busy || candidate.state !== 'review_ready'}><CheckCircle2 size={14} /> {busy ? '正在检查并保存…' : '接收并保存'}</button></>}<button type="button" className="mcu-diff-close" onClick={() => setShowDiff(false)} aria-label="返回代码" title="返回代码编辑器"><X size={16} /></button></div></header><DiffReview candidate={candidate} diff={props.candidateDiff} loading={props.candidateDiffLoading} error={props.candidateDiffError} history={props.workspaceHistory} busy={busy} onReject={props.onRejectCandidate} onBuild={props.onBuildCandidate} onApply={props.onApplyCandidate} onUndo={props.onUndoWorkspace} surfaceOnly /></div> : undefined
  const artifactCurrent = isFirmwareArtifactCurrent(props.build, workspace)
  const flashProgram = (): void => {
    openPanel('terminal')
    if (['connected', 'bootloader'].includes(props.connection.updatePort.state)) props.onStartUpdate()
    else props.onFlashWchLink()
  }
  const workspaceAction = {
    onCompile: () => { openPanel('terminal'); props.onBuildFirmware() },
    compileDisabled: busy || props.build.state === 'running',
    onFlash: flashProgram,
    flashDisabled: busy || !artifactCurrent,
    flashTitle: artifactCurrent ? '烧录当前编译产物；烧录前会自动检测开发板。' : '请先编译当前代码。'
  }
  const workspaceNotice = flashNotice(workspace.id, artifactCurrent, props.wchLink, props.update)

  return <section ref={shellRef} className={`mcu-workbench-shell ${hasGuide ? 'has-lab-guide' : 'is-sandbox'} ${workspace.platform === 'ti-mspm0' ? 'is-ti-mspm0' : ''} ${lectureFocus ? 'is-reference-expanded' : ''} ${guideDrawerOpen ? 'is-guide-drawer-open' : ''} ${explorerOpen ? 'is-explorer-open' : ''}`} style={{ '--mcu-explorer-width': `${explorerWidth}px`, '--mcu-guide-width': `${guideWidth}px` } as CSSProperties}>
    <div className="mcu-development-area">
      ...
      {workspace.platform === 'ti-mspm0' && <button type="button" className="mcu-sysconfig-button" onClick={props.onOpenSysconfig} disabled={busy}><Cpu size={16} /> 配置芯片 · SysConfig</button>}
      <StudentCodeEditor key={workspace.id} workspace={workspace} candidate={candidate} busy={busy} onCandidateChanged={props.onCandidateChanged} onReadyForReview={() => setShowDiff(true)} onRejectCandidate={props.onRejectCandidate} onExplainCode={explain} diagnosticHelp={props.diagnosticHelp} onRepairStudentCode={props.onRepairStudentCode} explorerMode focusRequest={focusRequest} onActiveFileChange={setActiveFilePath} editorOverlay={diffLayer} overlayVisible={Boolean(diffLayer)} workspaceAction={workspaceAction} workspaceNotice={workspaceNotice} workspaceDiagnostics={props.build.workspaceId === workspace.id ? props.build.diagnostics : []} bottomPanel={<McuBottomPanel key={workspace.id} workspace={workspace} candidate={candidate} build={props.build} baseline={props.baseline} connection={props.connection} update={props.update} wchLink={props.wchLink} busy={busy} request={panelRequest} onFocusFile={focusFile} onExplain={explain} onBuildFirmware={props.onBuildFirmware} onCancelBuild={props.onCancelBuild} onToggleUsb={props.onToggleUsb} onStartUpdate={props.onStartUpdate} onCancelUpdate={props.onCancelUpdate} onFlashWchLink={props.onFlashWchLink} onCancelWchLink={props.onCancelWchLink} />} />
      <button type="button" className="mcu-explorer-resizer" aria-label="调整工程目录宽度" onPointerDown={(event) => resize('explorer', event)} />
      {hasProjectTour && tourOpen && <ProjectTour index={tourIndex} onFocus={focusFile} onNext={() => { if (tourIndex < PROJECT_TOUR.length - 1) setTourIndex((value) => value + 1); else setTourOpen(false) }} onClose={() => setTourOpen(false)} />}
    </div>
...
export function flashNotice(workspaceId: string, artifactCurrent: boolean, wchLink: WchLinkFlashSnapshot, update: FirmwareUpdateSnapshot): { title: string; text: string; tone: 'info' | 'success' | 'error' } | undefined {
  if (wchLink.artifact?.workspaceId === workspaceId) {
    if (wchLink.state === 'failed') {
      const text = wchLink.error ?? wchLink.message
      return { title: /没有识别|未识别|没有检测|未检测|连接/.test(text) ? '未检测到开发板' : '烧录失败', text, tone: 'error' }
    }
    if (wchLink.state === 'completed') return { title: '烧录成功', text: wchLink.message, tone: 'success' }
    if (['probing', 'flashing', 'verifying', 'resetting'].includes(wchLink.state)) return { title: '正在烧录', text: wchLink.message, tone: 'info' }
  }
  if (!artifactCurrent || ['idle', 'cancelled'].includes(update.state)) return undefined
  if (update.state === 'failed') {
    const text = update.error ?? update.message
    return { title: /没有识别|未识别|没有检测|未检测|连接/.test(text) ? '未检测到开发板' : '烧录失败', text, tone: 'error' }
  }
  if (update.state === 'completed') return { title: '烧录成功', text: update.message, tone: 'success' }
  return { title: '正在烧录', text: update.message, tone: 'info' }
}

const PROJECT_TOUR_SEEN_KEY = 'robotdog.mcu-project-tour-seen'
...
const PROJECT_TOUR = [
  ...
  { path: 'App/Src/experiment.c', title: '你的实验代码', copy: '课程主要在 App 目录练习；修改后点击“保存”，长时间停顿时系统也会做一次兜底保存。' },
