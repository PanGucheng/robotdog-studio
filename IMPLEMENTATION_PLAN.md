...
- GCC 与 OpenOCD 均使用沁恒定制版本。开发阶段支持用户提供的本机安装位置，发布阶段使用用户提供的安装包和安装方式制作离线资源；纳入安装包前检查再分发许可、版本和 SHA-256。
- Git 提交按可独立验证的里程碑进行；可由 Codex 主动提交，但未经明确要求不推送远端。

当前固件现状需要特别记录：工程使用 MounRiver/Eclipse `.cproject`，没有 Makefile 或 CMake 命令行入口；动作和命令主要位于 `User/main.c`；命令应答走 USART3，而 `printf`、状态和 CCD 默认走 USART1。所以上位机 MVP 1 可先兼容当前 USART3 动作命令，真实 CCD 与完整一键编译闭环需等待下位机完成统一串口和命令行构建适配。
...
- 开发模式允许在设置中选择沁恒 GCC 安装目录；路径通过工具版本探测和试编译验证后保存。
- 发布模式优先从 `resources/tools/wch-gcc` 使用固定工具链，不依赖系统 PATH。
- Main 使用可执行文件与参数数组启动进程，不通过 Shell 拼接命令。
- 构建目录固定为工作区 `.rds/build`，输出 ELF、HEX、BIN、size 信息和结构化 GCC diagnostics。
- 支持取消、清理、原始日志和面向学生的中文错误解释。
- 每个产物绑定源码树哈希；工作区变化后旧产物立即失效。

在下位机尚未交付命令行构建适配前，使用模拟工具链和固定测试固件开发、测试上位机，不把 `.cproject` 解析器作为长期方案。

## 7. 烧录服务

- 使用只读 `board-profile.json` 定义沁恒 OpenOCD 可执行文件、WCH-Link 配置、目标芯片和允许参数。
- 开发模式支持选择用户提供的沁恒 OpenOCD 目录并运行版本、配置和探针检查。
- 发布模式从 `resources/tools/wch-openocd` 启动固定版本，不使用系统同名程序。
...
- 接入用户提供的沁恒 GCC/OpenOCD、结构化构建诊断、WCH-Link 探测和安全烧录。
- 在下位机交付正式构建接口前使用固定测试工程验证。
- 验收：全新 Windows 11、无 Node、Python 或 MounRiver 环境时，离线包可以构建并完成校验烧录。
