...
| 动作 | 它能说明什么 | 它不能说明什么 |
| --- | --- | --- |
| 保存 | 修改已经写入当前 Workspace | 代码能够通过编译 |
| 生成程序 | Compiler 与 Linker 能否把完整工程生成 ELF、HEX、BIN、MAP | 程序已经进入开发板 |
...
1. 阅读工程与实验入口；
2. 直接修改允许编辑的文件，等待自动保存；
3. 生成当前 Workspace 的完整程序；
4. 在 Terminal 阅读 Compiler 与 Linker 输出；
5. 有错误时定位、修复并重新生成；
6. 用自己的话总结保存、构建和烧录的区别。
...
源代码需要经过编译和链接才能成为程序文件；保存、生成程序和写入开发板是三个不同阶段。自己写的代码直接保存到 Workspace，AI 提议的代码修改则需要先阅读 Diff 再应用。
