...
状态：课程框架首个可用版本；关键状态失效、示例模板和扩展映射问题已修正，第三课继续由真机发布门禁阻止创建和发布；代码优先工作台作为下一阶段待实施
...
- 自动化测试集中覆盖数据读取、工作区创建和安全关键路径；普通界面点击可由开发侧代验，真机接线、烧录和硬件现象仍需具备实物条件时复核。
...
以下 1—9 项是制定计划时记录的缺口，现均已由阶段一至五解决；第 10 项串口终端仍不作为课程自动验收能力，界面和课程规则不会伪装它已经存在。
...
实现将轻量进度保存到应用受管数据根目录的 `course-progress/<workspaceId>.json`，与工作区身份一一对应并位于学生 `project/` 外，因此不进入代码 Diff 和 Git。实际 schema 保留以下等价信息：
...
- 代码再次应用修改或执行撤销时，不追溯提交哈希，但把受影响的候选预检、完整固件和烧录结果标为 `stale`，课程转为“需处理”；阅读、回答等学习记录继续保留。
- 进度写入按工作区串行执行，避免回答保存和编译回执同时写入时互相覆盖。
- 问题步骤通过 `questionId` 显式关联，人工观察完成条件通过目标 `stepId` 关联；不得用任意回答或任意观察满足多个步骤。
...
- [x] 前置课采用软门禁：未完成时提示并要求确认跳过，但允许大学生直接练习自包含课次。
...
- [x] 将工作区 schema 升级到 v3，实现一次性 v2→v3 非破坏迁移并保留旧元数据备份。
- [x] 现有 MCU 项目标记为 `mcu-sandbox`；现有趣味项目保持 `fun-project`。
- [x] 引入 `WorkspaceCreationSpec`，由 Main 根据 lesson ID 解析模板、策略和当前 `contentVersion`。
- [x] 改造 `WorkspaceService`，保留普通项目入口，同时新增 `createLessonAttempt`。
- [x] 为每课建立独立模板目录；课 0 使用基础模板，课 1 使用不同多文件模板，课 2 使用明确占位模板。
- [x] 课次 manifest 的 editable/readable/denied 规则与平台全局禁止规则取安全交集，再写入受管 `robotdog.project.json`。
- [x] 模板复制前检查目录存在、必要文件、符号链接和路径安全；失败时清理本次未完成目录。
- [x] 生成单调递增 `attemptNumber`，新练习不覆盖旧工作区。
- [x] “重新开始”实现为创建新尝试并保留旧尝试；不提供静默原地清空。
- [x] 课程中心显示继续上次、查看所有尝试、新建练习；普通 MCU sandbox 仍可使用但不显示课程完成状态。
- [x] Main 保证课程只选择 catalog 中登记的模板，候选区继续使用现有工作区身份和权限校验。

自动化验证：元数据迁移、模板缺失、路径越界、不同课模板、重复尝试、候选权限和 Git 隔离测试。

代验结果：已在无安装开发预览中创建第一课两次、第二课一次，确认旧尝试保留、多文件模板可逐项切换、继续和新建练习入口正确；桌面 IPC、工作区落盘及完整固件构建由 Electron 冒烟覆盖。详见 [阶段二复核记录](./manual-review-mcu-course-stage-2.md)。
...
- [x] 实现 `CourseProgressStore`，保存步骤勾选、问题答案、人工观察以及最近编译/烧录状态。
- [x] 实现少量固定 completion check，不建设通用规则引擎或证据注册表。
- [x] 为课程工作区初始化所有 step 状态，默认进入第一项未完成步骤。
- [x] MCU 工作台新增“实验任务”并作为课程工作区默认页；sandbox 仍默认工程代码。
- [x] 任务页展示目标、步骤、完成比例、最近操作状态、硬件要求、预期现象、下一步按钮、总结和问题。
- [x] 使用 route ID 统一跳转到代码、编译、Diff、烧录和资源页，移除依赖中文标签的跨页控制。
- [x] 从候选预检、完整固件和烧录服务读取最近一次结果，保存 `status + checkedAt`；不复制提交号、候选 ID或产物哈希。
- [x] 阅读、编辑、Diff、总结等步骤允许学生自行勾选；问题回答只检查非空和合理长度。
- [x] 对明确要求构建或烧录的课次，最近一次相关操作成功才允许整课标记完成；普通步骤不做防作弊设计。
- [x] 任务页注明进度记录仅供学习参考；代码应用或撤销后保留学习笔记，但将受影响的操作结果失效并提示重新检查。
- [x] 未提供正式串口终端前，串口步骤显示不可用或由学生人工确认，不能显示为自动通过。
- [x] 提供尝试状态聚合：未开始、进行中、需处理、已完成，以及完成时间。

自动化验证：进度重启持久化、并发写入串行化、损坏后备份重建、代码变化状态失效、多问题/多观察映射、最近编译/烧录状态、完成条件和进度不进入 Git Diff 测试。
...
- [x] 在 `CourseService` 中生成课程上下文；若后续逻辑明显膨胀，再拆出 `CourseContextService`。
- [x] 为修改、代码解释、编译诊断和总结四类请求生成相应上下文切片，避免无关内容占用模型上下文。
- [x] 上下文包含课程发布/硬件验证状态，第三课为 draft 时 AI 必须明确说明待验证。
- [x] 把课次上下文注入 `AgentSessionService` 的普通修改、选中代码解释、诊断解释和修复流程。
- [x] 更新 MCU 提示词版本，要求先提示、学生明确要求后再修改，不直接给整课最终答案。
- [x] 课次可编辑范围只用于解释；真正修改权限仍由 `PatchPolicyService` 和 Main 二次校验。
- [x] AI 回答不自动勾选课程步骤或伪造编译、烧录和硬件观察状态。
- [x] 保持 `deepseek-v4-flash` 唯一模型和现有 Reasonix 工作模式选择。
- [x] 对上下文做合理长度上限和基本文本清理。
...
- [x] 为每课写 manifest、独立模板、来源记录、权限和必要的完成检查 fixture。
- [x] 在开发模式完成选择→创建→任务→编辑→编译→诊断→AI→Diff→保存→固件→可选烧录→完成记录；两个无硬件课由增强 Electron 冒烟完整覆盖。
- [x] 记录课程文字和代码仍为示例状态，便于所有者后续逐课替换。
- [x] 验证课程页面只消费资源模型，新增普通课不需要修改核心页面流程；作者清单给出新增资源、模板和注册步骤。

自动化验证：两课无硬件端到端服务测试；第三课未验证状态显示测试。

人工复核：所有界面点击由项目所有者完成；出现问题时保存对应截图和日志，不要求每次成功流程都归档全套截图。

退出条件：两个无硬件课完整完成；硬件占位课诚实表达验证状态，并在真机检查前不能创建。后者是安全门禁的通过状态，不是软件缺口。
...
- [x] 新建 `docs/mcu-lesson-authoring-and-hardware-validation.md`，提供课程作者清单和模板骨架。
- [x] 建立 EVT 候选示例和来源记录；最终硬件方向确定后仍需在真机记录中补充所选示例的精确版本。
- [ ] 核对目标芯片、机器狗板原理图、实物、引脚复用、时钟、调试口、运动风险和供电。
- [x] 已提供受限硬件占位模板，不复制整个 EVT，也不开放启动/链接/Bootloader；正式实验转换必须等硬件方向确认。
- [x] 运行课程资源校验、候选预检、完整固件构建、程序资源阈值和打包后离线构建。
- [ ] 由项目所有者执行 WCH-Link/目标下载方式烧录、复位、现象观察、异常恢复和物理安全验证。
- [x] 在作者文档中定义硬件型号、接线说明、测试版本、实际现象、问题和验证日期的简明记录格式；实际记录在真机测试时填写。
- [x] 自动化测试覆盖完成条件的通过、失败与损坏恢复路径。
- [ ] 关键现象、复位和异常恢复通过后，将 `verification` 改为 `hardware-checked`，递增 `contentVersion` 并发布。
- [x] 更新架构、开发交接、完成复核记录，并生成双发行便携测试包。

自动化验证：课程 schema、引用文件齐全、正式包不显示 draft、趣味版不携带课程资源。

人工复核：驱动、接线、真实烧录、串口/硬件现象、异常恢复和卸载共存测试。

退出条件：新增课程已有可重复的简明流程；正式硬件课仍必须在具备实物后补齐真机检查记录，当前保持 draft 且不能创建或发布。
...
- 两个无硬件示例课可以完成端到端流程；
- 一个硬件课框架已安全展示；未真机验证时明确标记且禁止创建、接线和烧录；
...
- [x] 开发侧已代替项目所有者完成课程列表和阶段二交互复核，记录见 [阶段二复核记录](./manual-review-mcu-course-stage-2.md)。

第一批切片以及阶段二至阶段五的软件任务均已完成。阶段六的作者流程、自动校验、打包和交接也已完成；只保留必须具备 RobotDog 实物与最终原理图才能执行的第三课真机发布门禁，详见[课程框架完成复核记录](./manual-review-mcu-course-completion.md)。

## 10. 下一阶段：代码优先工作台与完整工程视图

2026-08-07 已确认大学生版下一阶段界面方向：

- 进入课次工程后，左侧工程目录和中央代码编辑器组成持续存在的代码主区；
- 右侧只保留课程、构建与运行、AI 助教三个工具，设置由顶栏打开；
- 修改确认在代码主区显示 Diff，候选预检、完整构建、资源摘要和烧录合并为按状态展开的“构建与运行”；
- 默认移除工具链组件、本机输出路径、成组产物卡、提交/输入哈希和成功时的完整日志，只在异常或技术详情中按需展示；
- 文件栏升级为工作区教学覆盖层与绑定固件基线的虚拟合并工程树；
- `App` 教学代码按现有策略可编辑，主固件、外设库、启动、链接和构建配置只读；
- 默认隐藏 Git、编辑器配置、缓存、构建临时目录、候选内部目录和固件产物；
- 课程步骤、编译诊断和 AI 解释均可定位当前文件/行号，同时不卸载编辑器或丢失草稿状态；
- 趣味巡线版保持现有低复杂度工作台，两个发行版继续共享业务服务和安全闭环。

本阶段涉及 Renderer 信息架构、虚拟文件数据模型、Main/IPC/Preload 按需读取和回归测试，不作为单纯 CSS 调整实施。完整分期、接口、安全边界和人工复核要求见[单片机入门版代码优先工作台改造计划](./mcu-code-first-workbench-redesign-plan.md)。
