...
状态（2026-07-14）：已完成并升级到 Reasonix CLI `v1.17.12`。源码以 Git 子模块固定到 `v1.17.12`（`afdcd161`），Windows x64 发布包和二进制 SHA-256 记录在 `config/reasonix-runtime.json`。应用已接入 ACP、候选目录权限策略、进程校验/脱敏/超时、safeStorage 密钥设置入口和 Mock 协议夹具；Reasonix runtime profile 由任务类型自动选择，不在学生界面暴露；真实联网提示仍需用户配置自己的 DeepSeek API Key 后人工验收。
...
状态（2026-06-20）：已完成软件闭环。界面可展示候选 Diff、真实 WCH GCC 预检证明、应用按钮和 Git 历史；应用前会重新核对正式 HEAD、工作区洁净状态、策略版本、源码树哈希和 Diff 哈希。应用以本地提交形成检查点，撤销通过 `git revert` 保留完整历史；提交失败与应用中断均有回滚或启动恢复测试。

当前预检范围是学生工作区中的 `student_control.c` 与 `line-following.yaml`。它证明受控学生代码可由内置沁恒 GCC 编译且配置合法，但不冒充完整 CH32V203 固件构建；完整链接、固件映像和下载产物将在最终硬件工程模板映射确定后接入。
...
状态（2026-06-20）：本阶段中与当前软件范围有关的异常恢复、浏览器端完整流程和自动化回归已完成；依赖真实串口、目标板、标准用户安装包及最终固件模板的验收继续保留到硬件阶段。
