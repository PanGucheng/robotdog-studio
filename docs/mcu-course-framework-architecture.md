...
当前可用版本仍保留项目选择器、工程代码、候选修改、编译、烧录和设置页。下一阶段已经确认改为代码优先工作台：进入课次工程后，由左侧工程树和中央编辑器组成的代码主区持续存在，课程、构建、修改确认、烧录、资源和 AI 助教迁入右侧工具坞。

该重构还将把当前少量 `StudentCodeFile[]` 平铺列表升级为受 Main 控制的虚拟合并工程树：
